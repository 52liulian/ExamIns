    ...
    def init_exam_times(self):
        ...
        if self.exam_type == "gk":
            ...
            if self.index == 0:  # 语文
                ...
                self.logger.info(f"设置考试时间: 开始={self.start_time.strftime('%H:%M:%S')}, 结束={self.end_time.strftime('%H:%M:%S')}")
            elif self.index == 2:  # 外语
                self.start_time = datetime.now().replace(hour=15, minute=0, second=0, microsecond=0)
                self.end_time = datetime.now().replace(hour=17, minute=0, second=0, microsecond=0)
                self.logger.info(f"设置考试时间: 开始={self.start_time.strftime('%H:%M:%S')}, 结束={self.end_time.strftime('%H:%M:%S')}")
            elif self.index == 3 or self.index == 4:  # 物理、历史
                self.start_time = datetime.now().replace(hour=9, minute=0, second=0, microsecond=0)
                self.end_time = datetime.now().replace(hour=10, minute=15, second=0, microsecond=0)
                self.logger.info(f"设置考试时间: 开始={self.start_time.strftime('%H:%M:%S')}, 结束={self.end_time.strftime('%H:%M:%S')}")
            elif self.index == 5:  # 化学
                self.start_time = datetime.now().replace(hour=8, minute=30, second=0, microsecond=0)
                self.end_time = datetime.now().replace(hour=9, minute=45, second=0, microsecond=0)
                self.logger.info(f"设置考试时间: 开始={self.start_time.strftime('%H:%M:%S')}, 结束={self.end_time.strftime('%H:%M:%S')}")
            elif self.index == 6:  # 生物
                self.start_time = datetime.now().replace(hour=17, minute=0, second=0, microsecond=0)
                self.end_time = datetime.now().replace(hour=18, minute=15, second=0, microsecond=0)
                self.logger.info(f"设置考试时间: 开始={self.start_time.strftime('%H:%M:%S')}, 结束={self.end_time.strftime('%H:%M:%S')}")
            elif self.index == 7:  # 地理
                self.start_time = datetime.now().replace(hour=11, minute=0, second=0, microsecond=0)
                self.end_time = datetime.now().replace(hour=12, minute=15, second=0, microsecond=0)
                self.logger.info(f"设置考试时间: 开始={self.start_time.strftime('%H:%M:%S')}, 结束={self.end_time.strftime('%H:%M:%S')}")
            elif self.index == 8:  # 政治
                self.start_time = datetime.now().replace(hour=14, minute=30, second=0, microsecond=0)
                self.end_time = datetime.now().replace(hour=15, minute=45, second=0, microsecond=0)
                self.logger.info(f"设置考试时间: 开始={self.start_time.strftime('%H:%M:%S')}, 结束={self.end_time.strftime('%H:%M:%S')}")
